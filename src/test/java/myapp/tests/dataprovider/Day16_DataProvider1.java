...
public class Day16_DataProvider1 {
    ...
    //TEST METHOD IS CONNECTED TO THE DATA PROVIDER METHOD BY USING THE METHOD NAME
    ...
    @DataProvider(name="management_credentials") //giving a custom name by using name annotation
    ...
    //TEST METHOD 2
    @Test(dataProvider = "management_credentials", groups = "minor_regression_group") //calling the data provider method with custom name
    ...
    //If the data is coming from a different class we must use dataProviderClass= ClassName.class
    //TEST METHOD 3
    @Test(dataProvider = "employeeCredentialsProvider", dataProviderClass = DataProviderUtils.class)
    public void employeeCredentialsTest(String email, String password){
    ...
    public void excelEmployeeDataTest(String email, String password){
        System.out.println("Email " + email + " | Password " + password);
    }
